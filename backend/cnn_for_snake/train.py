...
from simulate_snake import Simulate_snake
...
env = Simulate_snake(0)
brain = Brain((env.ncols, env.nrows, nLastStates), learningRate)
model = brain.model
...
def resetStates():
     ...
     currentState = np.zeros((1, env.ncols, env.nrows, nLastStates))
     
     for i in range(nLastStates):
          currentState[0, :, :, i] = env.grid
...
while True:
     ...
     while not gameOver:
          ...
          frame = np.reshape(frame, (1, env.ncols, env.nrows, 1))
     ...
     if epoch % 100 == 0 and epoch != 0:
          print("showing total collected",totNCollected)
          y = input("continue?")
